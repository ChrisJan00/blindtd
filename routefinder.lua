...
function findRoute(from, to, map)
	local routeslist = List.newList()
	local visitedlist = List.newList()
	local found = false
	...
	-- push starting pos
	List.pushFront(routeslist, {{from[1],from[2]},{from[1],from[2]},0},distanceMh(from,to))
	visited_marks[from[1]][from[2]]=1

	while routeslist.n>0 do
		local item = List.popFront(routeslist)
		List.pushBack(visitedlist, item)
		...
		if elem[2]>1 and (map[elem[1]][elem[2]].u==1 or map[elem[1]][elem[2]].u==2)
			...
			List.pushFrontSorted(routeslist,{{elem[1],elem[2]-1},{elem[1],elem[2]},steps+1},newdist)
		...
		if elem[1]>1 and (map[elem[1]][elem[2]].l==1 or map[elem[1]][elem[2]].l==2)
			...
			List.pushFrontSorted(routeslist,{{elem[1]-1,elem[2]},{elem[1],elem[2]},steps+1},newdist)
	...
	-- the last pushed item is the destination
 	local item = List.popBack(visitedlist)
	...
	while not (nextpoint[1]==from[1] and nextpoint[2]==from[2]) do
		...
		while not (item[1][1]==nextpoint[1] and item[1][2]==nextpoint[2]) do
			item = List.getPrev(visitedlist)
			if not item then item = List.getLast(visitedlist) end
		end
		-- push it to the path
		nextpoint = {item[2][1],item[2][2]}
		table.insert(path,1,{nextpoint[1],nextpoint[2]})
		-- we can skip it the next time we are searching
		List.removeCurrent(visitedlist)
	end

	List.discard(visitedlist)
